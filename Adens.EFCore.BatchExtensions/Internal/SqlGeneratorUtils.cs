...
namespace Adens.EFCore.BatchExtensions.Internal
{
    ...
    {
        public static Expression VisitSelect(IAdensQuerySqlGenerator sqlGenerator, ISqlGenerationHelper _sqlGenerationHelper, SelectExpression selectExpression)
        {
			...
			if (selectExpression.Projection.Any())
			{
				...
				{
					var oldSQL = Sql.Build().CommandText;//Adens's code
					sqlGenerator.Visit(e);
					var newSQL = Sql.Build().CommandText;
					string column = BatchUtils.Diff(oldSQL, newSQL); //Adens's code
					sqlGenerator.ProjectionSQL.Add(column); //Adens's code
				});
			}
			else
			{
				...
				//fix: https://github.com/adensW/Adens.EFCore.BatchExtensions/issues/40
				Sql.Append("1");
			}
			/*
			else
			{
				Sql.Append("1");
				sqlGenerator.ProjectionSQL.Add("1");//Adens's code
			}*/
			...
			if (selectExpression.Predicate != null)
			{
				...
				sqlGenerator.PredicateSQL = BatchUtils.Diff(oldSQL, Sql.Build().CommandText); //Adens's code
			}
